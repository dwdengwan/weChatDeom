...
var fContentArr = [];
let oldFcontentArr = JSON.parse(localStorage.getItem('fContentArrChild'));
if (oldFcontentArr !== null){
    cilckNum = oldFcontentArr.clickNum;
    fContentArr = JSON.parse(localStorage.getItem('fContentArr'));
    let fArr = JSON.stringify(fContentArr)
    localStorage.setItem('fContentArr',fArr)
    fContentArr[cilckNum] = oldFcontentArr;
}else{
    fContentArr = longlongagoArr;
...
fContentArr.forEach((item,i,arr)=>{
    ...
    <div class="friend-content-child">
        ...
        <div class="friend-content-child-text">
            <div class="friend-content-title">
                <a href="../../html/found/frienddetail.html" class="friend-content-a">
    ...
    aClick = document.getElementsByClassName('friend-content-a');
    ...
    console.log(oldFcontentArr.clickNum)
    //详情里点赞状态 在此页面也是点赞状态。
    if (item.status == 1){
        loveNum[item.clickNum].classList.add('active');
    } else{
        loveNum[item.clickNum].classList.remove('active');
    }
})
...
fContentArr.forEach((item,i,arr)=>{
    loveNum[i].addEventListener('click',function (e) {
        ...
        let arrParent = JSON.stringify(fContentArr);
        localStorage.setItem('fContentArr',arrParent);
    })
    aClick[i].addEventListener('click',function (e) {
        fContentArr[i].clickNum = i;
        let arr = JSON.stringify(fContentArr[i]);
        localStorage.setItem('fContentArrChild',arr);
    })
